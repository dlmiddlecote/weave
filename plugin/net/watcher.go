...
func (w *watcher) ContainerConnected(id, networkID string) {
	...
	// check that it's on our network
	network, err := w.driver.findNetworkInfo(networkID)
	if err != nil {
		w.driver.warn("ContainerConnected", "unable to find network %s info: %s", networkID, err)
		continue
	}
	if network.isOurs {
		fqdn := fmt.Sprintf("%s.%s", info.Config.Hostname, info.Config.Domainname)
...
func (w *watcher) ContainerDisconnected(id, networkID string) {}
